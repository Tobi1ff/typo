...
import {
  ...
  Plus, ArrowRight, X, MessageSquare, Ban, Briefcase, Users,
} from 'lucide-react';
import { formatDate, avatarUrl } from '../lib/utils';

interface Props { currentUser: User; }

type Status = 'none' | 'open_to_work' | 'looking_to_collaborate';

const STATUS_CONFIG: Record<Status, { label: string; color: string; icon: React.ReactNode }> = {
  none:                    { label: '',                      color: '',                      icon: null },
  open_to_work:            { label: 'Open to Work',          color: 'text-green-400 border-green-500/40 bg-green-900/10', icon: <Briefcase size={10} /> },
  looking_to_collaborate:  { label: 'Looking to Collaborate',color: 'text-blue-400 border-blue-500/40 bg-blue-900/10',   icon: <Users size={10} /> },
};
...
export default function Profile({ currentUser }: Props) {
  ...
  const [editData, setEditData]     = useState({
    displayName: '', bio: '', github: '', twitter: '', website: '', techStack: '',
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, 'users', uid)).then(snap => {
      ...
      setEditData({
        ...
        status:      (d.status as Status) || 'none',
      });
    }).catch(err => logError('Profile user', err));

    const postsQ = query(collection(db, 'posts'), where('authorUid', '==', uid), orderBy('createdAt', 'desc'));
    ...
    try {
      if (isFollowing) {
        ...
        await addDoc(collection(db, 'notifications'), {
          recipientUid: uid, senderUid: currentUser.uid,
          senderName: currentUser.displayName, type: 'follow',
  ...
  const statusCfg = STATUS_CONFIG[user.status as Status] ?? STATUS_CONFIG.none;
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      ...
      <div className="bg-[#111] border border-[#222] mb-8 overflow-hidden">
        ...
        <div className="p-6 pt-0 flex flex-col md:flex-row gap-6 -mt-12">
          <div className="relative shrink-0">
            <img src={user.photoURL || avatarUrl(user.uid || uid)} alt={user.displayName}
              className="w-28 h-28 border-4 border-[#0a0a0a] bg-[#111] object-cover" />
          </div>
              ...
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {user.status && user.status !== 'none' && (
                    <span className={`flex items-center gap-1.5 px-2.5 py-1 border text-[9px] font-mono uppercase tracking-wide ${statusCfg.color}`}>
                ...
                {isOwn ? (
                  ...
                  <>
                    <button onClick={handleMessage}
                      className="flex items-center gap-2 bg-[#111] border border-[#222] text-white px-4 py-2 hover:border-[#00ff00] hover:text-[#00ff00] transition-all font-mono text-xs">
                      <MessageSquare size={13} /> MESSAGE
                    </button>
                    <button onClick={handleFollow}
                      className={`flex items-center gap-2 px-5 py-2 font-bold transition-all active:scale-95 text-sm ${isFollowing ? 'bg-[#222] text-white hover:bg-red-900/20 hover:text-red-400' : 'bg-[#00ff00] text-black hover:bg-[#00cc00]'}`}>
                      {isFollowing ? 'DISCONNECT' : 'CONNECT'}
                    </button>
                    <button onClick={handleBlock}
                      className={`flex items-center gap-2 px-3 py-2 border text-xs font-mono transition-all ${isBlocked ? 'bg-red-900/20 text-red-400 border-red-500/40' : 'text-[#555] border-[#222] hover:text-red-400 hover:border-red-500/40'}`}
            ...
            <div className="mt-4">
              {isEditing ? (
                <div className="space-y-3 bg-[#0d0d0d] p-4 border border-[#222]">
                  ...
                  {/* Status picker */}
                  <div>
                    <p className="text-[9px] font-mono uppercase text-[#444] mb-2">Status</p>
                    <div className="flex flex-wrap gap-2">
                      {(Object.entries(STATUS_CONFIG) as [Status, typeof STATUS_CONFIG[Status]][]).map(([val, cfg]) => (
                        <button key={val} type="button"
                          onClick={() => setEditData(d => ({ ...d, status: val }))}
                          className={`flex items-center gap-1.5 px-3 py-1.5 border text-[9px] font-mono uppercase transition-all ${
                            editData.status === val
                              ? 'bg-[#00ff00] text-black border-[#00ff00]'
                              : 'text-[#555] border-[#222] hover:border-[#444] hover:text-white'
                          }`}
                        >
                          {val === 'none' ? 'None' : cfg.label}
                        </button>
                      ))}
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          {tab === 'posts' ? (
            loading
              ? <div className="h-48 bg-[#111] border border-[#222] animate-pulse" />
              : posts.length > 0
            ...
            <div className="space-y-5">
              ...
              {projects.length > 0
                ...
                : <div className="p-12 text-center border border-dashed border-[#222]">
                    <p className="text-[#444] font-mono text-sm uppercase tracking-widest">No projects yet.</p>
                  </div>
              }
            </div>
          )}
